...
let _ = init()
let make_NSXMLNode_of_NSXMLDocument (o : [`NSXMLDocument] nativeNSObject) = (Obj.magic o : [`NSXMLNode] nativeNSObject)
(* Class object for NSXMLDocument *)
let class_NSXMLDocument = object
   val repr = Classes.find "NSXMLDocument"
   method _new = (Objc.objcnew repr : [`NSXMLDocument] nativeNSObject)
   method _alloc = (Objc.objcalloc repr : [`NSXMLDocument] nativeNSObject)
   method replacementClassForClass (cls : [`NSObject] Objc.t) =
     (get_pointer (Objc.invoke Objc.tag_pointer repr (Selector.find "replacementClassForClass:")[make_pointer_from_object cls]) : [`NSXMLDocument] Objc.nativeNSObject)
end
...
class native_NSXMLDocument = fun (o : [`NSXMLDocument] nativeNSObject) -> object (self)
   inherit native_NSXMLNode (make_NSXMLNode_of_NSXMLDocument o) as super
   ...
   method initWithRootElement (element : [`NSXMLElement] Objc.t) =
     (get_pointer (Objc.invoke Objc.tag_pointer repr (Selector.find "initWithRootElement:")[make_pointer_from_object element]) : [`NSObject] Objc.nativeNSObject)
   method setCharacterEncoding (encoding : [`NSString] Objc.t) =
     (get_unit (Objc.invoke Objc.tag_unit repr (Selector.find "setCharacterEncoding:")[make_pointer_from_object encoding]) : unit)
   method characterEncoding =
     (get_pointer (Objc.invoke Objc.tag_pointer repr (Selector.find "characterEncoding:")[]) : [`NSString] Objc.nativeNSObject)
   method setVersion (version : [`NSString] Objc.t) =
     (get_unit (Objc.invoke Objc.tag_unit repr (Selector.find "setVersion:")[make_pointer_from_object version]) : unit)
   method version =
     (get_pointer (Objc.invoke Objc.tag_pointer repr (Selector.find "version:")[]) : [`NSString] Objc.nativeNSObject)
   method setStandalone (standalone : bool) =
     (get_unit (Objc.invoke Objc.tag_unit repr (Selector.find "setStandalone:")[make_bool standalone]) : unit)
   method isStandalone =
     (get_bool (Objc.invoke Objc.tag_bool repr (Selector.find "isStandalone:")[]) : bool)
   method setDocumentContentKind (kind : int) =
     (get_unit (Objc.invoke Objc.tag_unit repr (Selector.find "setDocumentContentKind:")[make_int kind]) : unit)
   method documentContentKind =
     (get_int (Objc.invoke Objc.tag_int repr (Selector.find "documentContentKind:")[]) : int)
   method setMIMEType (_MIMEType : [`NSString] Objc.t) =
     (get_unit (Objc.invoke Objc.tag_unit repr (Selector.find "setMIMEType:")[make_pointer_from_object _MIMEType]) : unit)
   method l_MIMEType =
   ...
   method setDTD (documentTypeDeclaration : [`NSXMLDTD] Objc.t) =
     (get_unit (Objc.invoke Objc.tag_unit repr (Selector.find "setDTD:")[make_pointer_from_object documentTypeDeclaration]) : unit)
   method l_DTD =
     (get_pointer (Objc.invoke Objc.tag_pointer repr (Selector.find "l_DTD:")[]) : [`NSXMLDTD] Objc.nativeNSObject)
   method setRootElement (root : [`NSXMLNode] Objc.t) =
     (get_unit (Objc.invoke Objc.tag_unit repr (Selector.find "setRootElement:")[make_pointer_from_object root]) : unit)
   method rootElement =
     (get_pointer (Objc.invoke Objc.tag_pointer repr (Selector.find "rootElement:")[]) : [`NSXMLElement] Objc.nativeNSObject)
   ...
   method replaceChildAtIndex  ~withNode:(node : [`NSXMLNode] Objc.t ) (index : int) =
     let sel, args = (
       ...
       (get_unit (Objc.invoke Objc.tag_unit repr (Selector.find_list sel) args) : unit)
   method l_XMLData =
     (get_pointer (Objc.invoke Objc.tag_pointer repr (Selector.find "l_XMLData:")[]) : [`NSData] Objc.nativeNSObject)
   method l_XMLDataWithOptions (options : int) =
     (get_pointer (Objc.invoke Objc.tag_pointer repr (Selector.find "l_XMLDataWithOptions:")[make_int options]) : [`NSData] Objc.nativeNSObject)
...
(*  UNSUPPORTED
   method validateAndReturnError (error : (*pointer to pointer to NSError*) unsupported) =
     (get_bool (Objc.invoke Objc.tag_bool repr (Selector.find "validateAndReturnError:")[make_error error]) : bool)
