...
let _ = init()
let make_NSObject_of_NSRunLoop (o : [`NSRunLoop] nativeNSObject) = (Obj.magic o : [`NSObject] nativeNSObject)
(* Class object for NSRunLoop *)
let class_NSRunLoop = object
   val repr = Classes.find "NSRunLoop"
   method _new = (Objc.objcnew repr : [`NSRunLoop] nativeNSObject)
   method _alloc = (Objc.objcalloc repr : [`NSRunLoop] nativeNSObject)
(* methods for category NSOrderedPerform *)
(* methods for category NSRunLoopConveniences *)
   method currentRunLoop =
     (get_pointer (Objc.invoke Objc.tag_pointer repr (Selector.find "currentRunLoop:")[]) : [`NSRunLoop] Objc.nativeNSObject)
end
(* Encapsulation for native instance of NSRunLoop *)
class native_NSRunLoop = fun (o : [`NSRunLoop] nativeNSObject) -> object (self)
   inherit native_NSObject (make_NSObject_of_NSRunLoop o) as super
(* methods for category NSOrderedPerform *)
   ...
   method cancelPerformSelectorsWithTarget (target : [`NSObject] Objc.t) =
     (get_unit (Objc.invoke Objc.tag_unit repr (Selector.find "cancelPerformSelectorsWithTarget:")[make_pointer_from_object target]) : unit)
(* methods for category NSRunLoopConveniences *)
   method run =
     (get_unit (Objc.invoke Objc.tag_unit repr (Selector.find "run:")[]) : unit)
   method runUntilDate (limitDate : [`NSDate] Objc.t) =
     (get_unit (Objc.invoke Objc.tag_unit repr (Selector.find "runUntilDate:")[make_pointer_from_object limitDate]) : unit)
   ...
   method configureAsServer =
     (get_unit (Objc.invoke Objc.tag_unit repr (Selector.find "configureAsServer:")[]) : unit)
   method currentMode =
     (get_pointer (Objc.invoke Objc.tag_pointer repr (Selector.find "currentMode:")[]) : [`NSString] Objc.nativeNSObject)
(*  UNSUPPORTED
   method getCFRunLoop =
     ((*CFRunLoopRef*) unsupported (Objc.invoke (*CFRunLoopRef*) Objc.tag_unsupported repr (Selector.find "getCFRunLoop:")[]) : (*CFRunLoopRef*) unsupported)
...
*)
   ...
   method limitDateForMode (mode : [`NSString] Objc.t) =
     (get_pointer (Objc.invoke Objc.tag_pointer repr (Selector.find "limitDateForMode:")[make_pointer_from_object mode]) : [`NSDate] Objc.nativeNSObject)
