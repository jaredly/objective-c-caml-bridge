...
(*  UNSUPPORTED
   method sizeWithAttributes (attrs : [`NSDictionary] Objc.t) =
     ((*NSSize*) unsupported (Objc.invoke (*NSSize*) Objc.tag_unsupported o (Selector.find "sizeWithAttributes:")[make_pointer_from_object attrs]) : (*NSSize*) unsupported)
...
(*  UNSUPPORTED
   method drawAtPoint (point : (*NSPoint*) unsupported) =
     (get_unit (Objc.invoke Objc.tag_unit o (Selector.find "drawAtPoint:")[(*NSPoint*) unsupported point]) : unit)

*)
(*  UNSUPPORTED
   method drawInRect (rect : (*NSRect*) unsupported) =
     (get_unit (Objc.invoke Objc.tag_unit o (Selector.find "drawInRect:")[(*NSRect*) unsupported rect]) : unit)
