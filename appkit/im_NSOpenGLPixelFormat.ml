...
(*  UNSUPPORTED
  method getValues_forAttribute_forVirtualScreen  (vals : (*pointer to long int*) unsupported) (attrib : int) (screen : int) =
    let sel, args = (
...
*)
  method numberOfVirtualScreens =
    (get_int (Objc.invoke Objc.tag_int self#repr (Selector.find "numberOfVirtualScreens")[])
       : int)
  method _CGLPixelFormatObj =
    ((get_pointer (Objc.invoke Objc.tag_pointer self#repr (Selector.find "CGLPixelFormatObj")[])
       : [`void] Objc.id))
end
