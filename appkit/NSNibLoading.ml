...
(* methods for category NSNibLoading *)
   method loadNibFile  ~externalNameTable:(context : [`NSDictionary] Objc.t ) ~withZone:(zone : [`NSZone] Objc.t ) (fileName : [`NSString] Objc.t) =
     let sel, args = (
       ...
       (get_pointer (Objc.invoke Objc.tag_pointer repr (Selector.find_list sel) args) : [`NSBundle] Objc.nativeNSObject)
...
(* methods for category NSNibAwaking *)
   method awakeFromNib =
     (get_unit (Objc.invoke Objc.tag_unit repr (Selector.find "awakeFromNib:")[]) : unit)
end
